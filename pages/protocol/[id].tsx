...
function formatData(data: any, minDate: string, maxDate: string, primary: string, secondary?: string | null) {
  const result = [];
  for (let date = new Date(minDate); !isAfter(date, new Date(maxDate)); date = addDays(date, 1)) {
    const dateStr = formatDate(date);
    result.push({
...
const useFees = (
  ...
  useEffect(() => {
    ...
    if (missingPrimary.length > 0 || missingSecondary.length > 0) {
      ...
      const secondaryQuery = missingSecondary.length > 0 ? `&${secondary}=${missingSecondary.join(',')}` : '';
...
export const ProtocolDetails: NextPage<ProtocolDetailsProps> = ({ id, metadata, feeCache, protocols }) => {
  ...
  return (
    <main>
      ...
      >
        <option>None</option>
        {protocols.map((id: string) => <option key={id}>{id}</option>)}
      </select>
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  const defaultFeesArray = await getDateRangeData(id, subDays(new Date(), 90), subDays(new Date(), 1));
